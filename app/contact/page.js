...
function EnquiryForm(){const product=useSearchParams().get('product');const match=products.find(p=>p[0]===product);return <>{product&&<div className="selected"><img src={match?match[3]:'/flower jaali wooden wall art.jpg'} alt="Selected product"/><div><small>SELECTED PRODUCT</small><strong>{product}</strong><span>Custom laser art</span></div></div>}<WhatsAppForm product={product}/></>}
